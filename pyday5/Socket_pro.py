...
def make_auth_log(p_attack: float, json_mode=False):
    ...
    if "Failed password" in msg :
        with open("FailedPasswords.log","a") as f :
             if json_mode:
                f.write(json.dumps(line) + "\n")
             else:
                f.write(
                    f'Aug {datetime.utcnow().day:02d} '
                    f'{datetime.utcnow().strftime("%H:%M:%S")} '
                    f'web01 sshd[1234]: {msg}\n'
                )
...
def tcp_head(host, port=80, path="/"):
    ...
    # عرض الاستجابة (رؤوس HTTP فقط)
    print(response.decode(errors="ignore"))

# مثال تشغيل
tcp_head("example.com")
'''
'''
    import socket
...
server_socket.bind((HOST, PORT))  # Bind IP and Port
...
while True:
    ...
    conn.sendall(data)  # Echo message back
